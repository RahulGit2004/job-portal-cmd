...
const JOB_ID = "67c9c913553e3d343f2216ec"; // Your job ID
const API_URL = `http://localhost:8000/api/v1/job/jobs/${JOB_ID}/applicants`;
  ...
  useEffect(() => {
    const fetchApplicants = async () => {
      try {
        ...
        localStorage.setItem(`job_${JOB_ID}`, JSON.stringify(data));
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="relative w-full">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {displayedApplicants.map((applicant, index) => (
                ...
                >
                  <img
                    src={applicant.image || profileImg}
                    alt={applicant.name}
                    className="h-12 w-12 rounded-full mb-2"
                  />
                  <h3 className="font-semibold">{applicant.name}</h3>
                  <p className="text-sm text-gray-500">• {applicant.role || "UI / UX Designer"}</p>
                  <p className="text-sm text-gray-500">• {applicant.experience} Experience</p>
                  <p className="text-sm text-gray-500">• Education: {applicant.education}</p>
                  <p className="text-sm text-gray-500">• Applied: {applicant.applied}</p>
                  <button className="mt-2 px-4 py-1 bg-green-500 text-white text-sm rounded-md self-start hover:bg-green-600 transition">
        ...
        <div className="hidden md:block bg-[#F8F9FA] rounded-lg shadow-lg p-4 md:p-6">
          <h2 className="text-lg font-bold mb-4">
            Shortlisted <span className="text-gray-500">{shortlisted.length}</span>
          </h2>
          <div className="grid grid-cols-1 gap-4">
            {shortlisted.map((applicant, index) => (
              <div
                key={index}
                className="bg-white shadow-md rounded-lg p-4 border flex flex-col hover:bg-blue-100 transition"
              >
                <img
                  src={applicant.image || profileImg}
                  alt={applicant.name}
                ...
                <p className="text-sm text-gray-500">• Education: {applicant.education}</p>
                <p className="text-sm text-gray-500">• Applied: {applicant.applied}</p>
                <button className="mt-2 px-4 py-1 bg-red-500 text-white text-sm rounded-md self-start hover:bg-red-600 transition">
                  Reject
                </button>
              </div>
            ))}
